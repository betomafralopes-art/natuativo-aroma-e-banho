'use client'

import Link from 'next/link'
import { useState, useEffect } from 'react'
import { Menu, X, Search } from 'lucide-react'
import { cn } from '@/lib/utils'
import CartIcon from '@/components/commerce/cart/CartIcon'
import CartDrawer from '@/components/commerce/cart/CartDrawer'
import { useCart } from '@/components/commerce/cart/useCart'

export default function Header() {
  const [mobileOpen, setMobileOpen] = useState(false)
  const [scrolled, setScrolled] = useState(false)
  const [cartOpen, setCartOpen] = useState(false)
  const { totalItens } = useCart()


  useEffect(() => {
    const onScroll = () => setScrolled(window.scrollY > 40)
    window.addEventListener('scroll', onScroll, { passive: true })
    return () => window.removeEventListener('scroll', onScroll)
  }, [])

  return (
    <header
      className={cn(
        'fixed top-0 inset-x-0 z-40 transition-all duration-300',
        scrolled
          ? 'bg-white/95 backdrop-blur-sm border-b border-cream-300 shadow-sm'
          : 'bg-transparent'
      )}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-[60px] md:h-20">

          {/* Logo */}
          <Link href="/" className="flex flex-col leading-none group">
            <span
              className="font-display text-[1.15rem] md:text-2xl font-semibold tracking-wide transition-colors"
              style={{ color: '#C9A84C' }}
            >
              Natuativo
            </span>
            <span className="font-body text-[10px] md:text-xs font-light tracking-[0.18em] text-charcoal-500 uppercase">
              Aroma & Banho
            </span>
          </Link>

          {/* Desktop Nav */}

          <nav className="hidden md:flex items-center gap-8">

            <Link
              href="/"
              className="font-body text-sm text-charcoal-600 hover:text-gold-500 transition-colors tracking-wide"
            >
              Início
            </Link>
            <Link
              href="/catalogo"
              className="font-body text-sm text-charcoal-600 hover:text-gold-500 transition-colors tracking-wide"
            >
              Catálogo
            </Link>
            <Link
              href="/catalogo?lancamentos=true"
              className="font-body text-sm text-charcoal-600 hover:text-gold-500 transition-colors tracking-wide"
            >
              Lançamentos
            </Link>
            <a
              href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '5548996350861'}?text=${encodeURIComponent('Olá! Gostaria de saber mais sobre os produtos da Natuativo Aroma & Banho.')}`}
              target="_blank"
              rel="noopener noreferrer"
              className="btn-gold text-xs px-5 py-2.5"
            >
              Fale Conosco
            </a>
          </nav>

          <div className="hidden md:flex items-center ml-5">

           <CartIcon
            quantidade={totalItens}
            onClick={() => setCartOpen(true)}
           />
         </div>

          {/* Mobile Actions */}
<div className="flex items-center gap-2 md:hidden">

  <CartIcon
    quantidade={totalItens}
    onClick={() => setCartOpen(true)}
  />

  <button
    onClick={() => setMobileOpen(!mobileOpen)}
    className="flex h-11 w-11 items-center justify-center rounded-full text-charcoal-600 transition-all duration-300 hover:bg-cream-100 hover:text-gold-500"
    aria-label="Menu"
  >
    {mobileOpen ? <X size={22} /> : <Menu size={22} />}
  </button>

</div>
        </div>
      </div>

      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-cream-300 shadow-lg animate-scaleIn">
          <nav className="max-w-7xl mx-auto px-5 py-5 flex flex-col gap-1">
            <Link
              href="/"
              onClick={() => setMobileOpen(false)}
              className="flex items-center h-12 font-body text-[15px] text-charcoal-700 hover:text-gold-500 transition-all duration-300 border-b border-cream-200"
            >
              Início
            </Link>
            <Link
              href="/catalogo"
              onClick={() => setMobileOpen(false)}
              className="flex items-center h-12 font-body text-[15px] text-charcoal-700 hover:text-gold-500 transition-all duration-300 border-b border-cream-200"
            >
              Catálogo
            </Link>
            <Link
              href="/catalogo?lancamentos=true"
              onClick={() => setMobileOpen(false)}
              className="flex items-center h-12 font-body text-[15px] text-charcoal-700 hover:text-gold-500 transition-all duration-300 border-b border-cream-200"
            >
              Lançamentos
            </Link>
            <a
              href="https://www.instagram.com/natuativoaromaebanho"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center h-12 font-body text-[15px] text-charcoal-700 hover:text-gold-500 transition-all duration-300 border-b border-cream-200"
            >
              Instagram
            </a>
            <a
              href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '5548996350861'}?text=${encodeURIComponent('Olá! Gostaria de saber mais sobre os produtos da Natuativo Aroma & Banho.')}`}
              target="_blank"
              rel="noopener noreferrer"
              className="btn-whatsapp mt-5 py-3"
            >
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
              Fale pelo WhatsApp
            </a>
          </nav>
        </div>
      )}

        <CartDrawer
        aberto={cartOpen}
        onClose={() => setCartOpen(false)}
        />
       

    </header>
  )
}
